...
public class ArtWorksNetworkAction extends BaseAction {

   private final Log logger = LogFactory.getLog(ArtWorksNetworkAction.class);
   ...
   public String pagePublication() throws Exception {
      ...
      try {
         ...
         pageQuery = artWorksNetworkService.queryArtWorksNetwork(pageQuery);
         if (pageQuery != null) {
            ...
            if (rs != null) {
               ...
               for (int i = 0; i < rs.size(); i++) {
                  ...
                  map.put(
                        "data",
                        new Object[] { "", publication.get("literatureTitle") + link, publication.get("literatureAuther"), publication.get("literatureLink"),
                              publication.get("literatureWorks"), publication.get("quoteLiterature"), publication.get("personInvolved"),
                              publication.get("relatedExhib"), publication.get("relatedEvent") });
                  list.add(map);
               }
               String res = genGridJson(pageQuery, list);
               artSysLogService.createArtSysLog(currentUser.getUserName(), "作品管理", "作品网媒关系管理", "查询条件为：" + QTool.getParamter(pageQuery.getParameters()));
               setResponse(res);
            }
   ...
   public String addpage() throws Exception {
      ...
      try {
         ...
         pq = artLiteratureNetworkService.queryArtLiteratureNetwork(pq);
         List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
         for (Map<String, Object> item : pq.getRecordSet()) {
            Map<String, Object> map = new HashMap<String, Object>();
            String link = "^javascript:view(" + item.get("id") + ");^_self";
            map.put("id", item.get("id"));
            map.put("data", new Object[] { "", item.get("literatureTitle") + link, item.get("literatureAuther"), item.get("literatureLink") });
            list.add(map);
         }
   ...
   public String savePublication() throws Exception {
      ...
      try {
         ...
         log.append("]");
         artWorksNetworkService.createArtWorksNetwork(worksId, add_ids);
         artSysLogService.createArtSysLog(currentUser.getUserName(), "作品管理", "作品网媒关系管理", "添加作品网媒关系：" + log.toString());
         setResponse("{exit:0}");
      }
      catch (Exception e) {
         e.printStackTrace();
   ...
   public String deletePublication() throws Exception {
      ...
      try {
         ...
         for (int i = 0; i < del_ids.length; i++) {
            ...
            ArtWorksNetwork artWorksNetwork = artWorksNetworkService.getArtWorksNetwork(del_ids[i]);
            ArtLiteratureNetwork artLiteratureNetwork = artLiteratureNetworkService.getArtLiteratureNetwork(artWorksNetwork.getNetworkId());
            ArtWorks artWorks = artWorksService.getArtWorks(artWorksNetwork.getWorksId());
            if (log.length() > 0) log.append(",");
            log.append(artWorks.getWorksCName()).append(":").append("[").append(artLiteratureNetwork.getLiteratureTitle()).append("]");
         }
         artWorksNetworkService.deleteArtWorksNetworks(del_ids);
         artSysLogService.createArtSysLog(currentUser.getUserName(), "作品管理", "作品网媒关系管理", "删除作品网媒关系：" + log.toString());
         setResponse("{exit:0}");
      }
      catch (ServiceException e) {
         setResponse("{exit:1,message:\"" + e.getMessage() + "\"}");
   ...
   @Resource
   private ArtLiteratureNetworkService artLiteratureNetworkService;

   @Resource
   private ArtWorksNetworkService      artWorksNetworkService;
