...
var myPager = new Web.Pager();//初始化分页
...
this.defaultHtml = "#{recordCount}#{pageIndex}#{pageSize}#{pageSizeSelect}#{pageCount}#{pageFrist}#{pagePrevious}#{pageText}#{pageNext}#{pageLast}#{pageSelect}#{pageInput}#{pageGo}";
	...
	write:function(id,mode,html){
		...
		if(this.getPageCount()<=0){
			...
			if(isPageFrist)
			sPageFrist = (pageIndex>1&&pageIndex<=pageCount) ? this.setActive(pageFrist[0],"首页-Frist","pageFrist",1):this.setDisable(pageFrist[1],"首页-Frist","dispageFrist");
			if(isPagePrevious)
			sPagePrevious =(pageIndex>1&&pageIndex<=pageCount) ? this.setActive(pagePrevious[0],"上一页-Previous","pagePrevious",prevPage):this.setDisable(pagePrevious[1],"上一页-pagePrevious","dispagePrevious");
			if(isPageNext)
			sPageNext = (pageIndex>0&&pageIndex<pageCount) ? this.setActive(pageNext[0],"下一页-Next","pageNext",nextPage):this.setDisable(pageNext[1],"下一页-Next","dispageNext");
			if(isPageLast)
			sPageLast = (pageIndex>0&&pageIndex<pageCount) ? this.setActive(pageLast[0],"尾页-Last","pageLast",pageCount):this.setDisable(pageLast[1],"尾页-Last","dispageLast");
			if(isPageInput)
			{
				...
				switch(pageTextMode)
				{
					...
					case 1:////0-100页输出页码模式
						...
						if (pageIndex <(pageCount-1)&&ePage<(pageCount-1))
						{
							...
							sPageText +=this.setActive(_t,"页码-"+(pageCount-1),"pageNum",pageCount-1);
						}
						if (pageIndex <pageCount&&ePage<pageCount)
						{
							var _t = pageText[1].replace(/#{pageIndex}/ig,pageCount);
							sPageText +=this.setActive(_t,"页码-"+pageCount,"pageNum",pageCount);
						}
						break;
					case 2://50-10000页输出页码模式
						...
						if (bPage>10)
						{
							sPageText +=this.setActive("...","上十页-Prev 10 Pages","pageDot",(bPage-1));
						}
						for (var i = bPage; i < ePage; i++) {
						...
						{
							sPageText +=this.setActive("...","下十页-Next 10 Pages","pageDot",(bPage + 10));	
						}
						if (pageIndex<pageCount&&ePage<pageCount)
						{
							var _t = pageText[1].replace(/#{pageIndex}/ig,pageCount);
