...
<script language="javascript">  
	var hyperlink = "../artist/ArtistEdu.do";	
	var fulllink = contextpath + "artist/ArtistEdu.do";		
      
	function goAdd()  {
		editMode="ADD";
		var artistId = $$('artistId').value;
	    openWindow("添加艺术家教育经历与工作",fulllink+'?action=ADD&record.artistId=' + artistId,0.6,0.4);
	}
	
	function view(id) {
	       var url_link=fulllink+'?action=VIEW&ids=' + id;
	       editMode="VIEW";
	       openWindow("查看艺术家教育经历与工作",url_link,0.6,0.4);	
	}
	
	function goModify(){
		var id = findSelected("修改",ArtistEdu_grid);
		if (id == "") return;
		url_link = fulllink+'?action=EDIT&ids='+id;
		editMode="EDIT";
	    openWindow("修改艺术家教育经历与工作",url_link,0.6,0.4);	  
	}

	function goDel()  {
		var id = findMultiSelected("删除",ArtistEdu_grid);
		if (id == "") return;
		var res = confirm("是否要删除？");
		if (res == true){
			var data = ajaxSubmit(fulllink+"?action=delete&ids=" + id,"ArtistEduForm");
			if(data.exit>0){
	 			  alert(data.message);
	 			  return;
	 		}else {	  
	...
	function goImport() {
		...
		openWindow("导入教育经历与工作",url_link,0.5,0.5);
	}
...
<body onload="init()">
	<div class="special-padding" id="mainDiv">
		<s:form action="ArtistEdu" id="ArtistEduForm" method="post" namespace="/artist" enctype="multipart/form-data">
...
<script type="text/javascript">
    if(window.addEventListener) {
	 ...
	 var gDiv=window.document.getElementById('ArtistEdu_box');
