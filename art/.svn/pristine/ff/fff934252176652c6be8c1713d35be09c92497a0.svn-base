...
<head>
   <%@include file="/common/header.jsp" %>
	...
	<script language="javascript">		
		var hyperlink = "../works/worksTheme.do";
		var fulllink = contextpath + "works/worksTheme.do";

		// 用于浏览该题材下的用户
		function view(id) {
			editMode="VIEW";
			openWindow("查看题材", fulllink + "?action=view&ids=" + id, 600, 300);
		}
   
		function goDel() {
			var id = findMultiSelected("删除",theme_grid);
			if(id == "") return;
			var res = confirm("是否真的要删除?");
			if(res == true) {
				var data=ajaxSubmit(fulllink+"?action=delete&ids=" + id,"themeForm");
		 		if(data.exit>0){
		 			  alert(data.message);
		 			  return;
		 		}
		...
		function goRowDblClick() {
			var btn = $$('btnModify');
			if(btn==null) {
				var id = findSelected("修改",theme_grid);
				if(id == "") return;
				view(id);
			}
			else {
		...
		function goModify() {
    		...
    		editMode="EDIT";
			openWindow("修改题材", urlink, 600, 300);
		}

		function goAdd() {
			editMode="ADD";
			openWindow("添加题材", fulllink+'?action=ADD', 600, 300);
		}
		...
		function renew() {
			...
			var v0 =    getElement("query.parameters.themeName");    v0.value="";
			goSearch();
		}
...
<s:form action="worksTheme" id="themeForm" method="post" namespace="/works">
...
<s:hidden name="query.pageCount" />
	...
	<tbody>
		<tr>
			<td class="textL">
  &nbsp;&nbsp;题材名称：<s:textfield style="width:200px;" name="query.parameters.themeName" /> 
...
<gl:grid styleClass="pageTurn" id="theme" page="true" form="themeForm"  property="query" cellPadding="0" cellSpacing="0" >
   var grid = new dhtmlXGridObject();
   grid.setImagePath(imagePath);
   var headAlign=[tLeft,tCenter,tCenter,tCenter,tCenter];
   grid.setHeader("#master_checkbox,题材名称,题材描述,创建人,创建时间",null,headAlign);
