...
<script language="javascript">  
	var hyperlink = "../auction/ArtAuctionWords.do";	
	var fulllink = contextpath + "auction/ArtAuctionWords.do";		
      
	function goAdd()  {
		editMode="ADD";
	    openWindow("添加拍卖相关文字",fulllink+'?action=ADD&record.auctionId='+$$('auctionId').value,0.6,0.6);
	}
	
	function view(id) {
	       var url_link=fulllink+'?action=VIEW&ids='+id;
	       editMode="VIEW";
	       openWindow("查看拍卖相关文字信息",url_link,0.6,0.6);	
	}
	
	function goModify(){
		var id = findSelected("修改",ArtAuctionWords_grid);
		if (id == "") return;
		url_link = fulllink+'?action=EDIT&record.id='+id;
		editMode="EDIT";
	    openWindow("修改拍卖相关文字信息",url_link,0.6,0.6);	  
	}

	function goDel()  {
		var id = findMultiSelected("删除",ArtAuctionWords_grid);
		if (id == "") return;
		var res = confirm("是否要删除？");
		if (res == true){
			var data = ajaxSubmit(fulllink+"?action=delete&ids=" + id,"ArtAuctionWordsForm");
			if(data.exit>0){
	 			  alert(data.message);
	 			  return;
	 		}else {	  
	...
	function goImport() {
		...
		openWindow("导入拍卖相关文字",url_link,0.5,0.5);
	}
...
<body onload="init()">
	<div class="special-padding" id="mainDiv">
		<s:form action="ArtAuctionWords" id="ArtAuctionWordsForm" method="post" namespace="/auction" enctype="multipart/form-data">
			...
			<gl:grid styleClass="pageTurn" id="ArtAuctionWords" page="true" form="ArtAuctionWordsForm"  property="query" cellPadding="0" cellSpacing="0">
				var grid = new dhtmlXGridObject();
         	    grid.setImagePath(imagePath);
				var headAlign = [tCenter,tCenter,tCenter,tCenter];
				grid.setHeader("#master_checkbox,文章主题,出处,时间",null, headAlign);
				grid.setInitWidthsP("5,40,35,20");
				grid.setColAlign("center,center,center,center");
				grid.setColTypes("ch,link,ro,ro");
				grid.setColSorting("na,str,str,str");
				grid.init();
				var gDiv = $$('ArtAuctionWords_box');
			</gl:grid>
		</s:form>
	</div>
...
<script type="text/javascript">
    if(window.addEventListener) {
	 ...
	 var gDiv=window.document.getElementById('ArtAuctionWords_box');
