...
<script language="javascript">  
	var hyperlink = "../artist/ArtistHonors.do";	
	var fulllink = contextpath + "artist/ArtistHonors.do";		
      
	function goAdd()  {
		editMode="ADD";
		var artistId = $$('artistId').value;
	    openWindow("添加艺术家获奖与荣誉",fulllink+'?action=ADD&record.artistId=' + artistId,0.6,0.4);
	}
	
	function view(id) {
	       var url_link=fulllink+'?action=VIEW&ids=' + id;
	       editMode="VIEW";
	       openWindow("查看艺术家获奖与荣誉",url_link,0.6,0.4);	
	}
	
	function goModify(){
		var id = findSelected("修改",ArtistHonors_grid);
		if (id == "") return;
		url_link = fulllink+'?action=EDIT&ids='+id;
		editMode="EDIT";
	    openWindow("修改艺术家获奖与荣誉",url_link,0.6,0.4);	  
	}

	function goDel()  {
		var id = findMultiSelected("删除",ArtistHonors_grid);
		if (id == "") return;
		var res = confirm("是否要删除？");
		if (res == true){
			var data = ajaxSubmit(fulllink+"?action=delete&ids=" + id,"ArtistHonorsForm");
			if(data.exit>0){
	 			  alert(data.message);
	 			  return;
	 		}else {	  
	...
	function goImport() {
		...
		openWindow("导入获奖与荣誉",url_link,0.5,0.5);
	}
...
<body onload="init()">
	<div class="special-padding" id="mainDiv">
		<s:form action="ArtistHonors" id="ArtistHonorsForm" method="post" namespace="/artist" enctype="multipart/form-data">
			...
			<gl:grid styleClass="pageTurn" id="ArtistHonors" page="true" form="ArtistHonorsForm"  property="query" cellPadding="0" cellSpacing="0">
				var grid = new dhtmlXGridObject();
         	    grid.setImagePath(imagePath);
				var headAlign = [tCenter,tCenter,tCenter,tCenter,tCenter];
				grid.setHeader("#master_checkbox,时间,事件",null, headAlign);
				grid.setInitWidthsP("5,30,65");
				grid.setColAlign("center,center,center");
				grid.setColTypes("ch,link,ro");
				grid.setColSorting("na,str,str");
				grid.init();
				var gDiv = $$('ArtistHonors_box');
			</gl:grid>
		</s:form>
	</div>
...
<script type="text/javascript">
    if(window.addEventListener) {
	 ...
	 var gDiv=window.document.getElementById('ArtistHonors_box');
