...
<script language="javascript">  
	var hyperlink = "../auction/ArtAuctionVideo.do";	
	var fulllink = contextpath + "auction/ArtAuctionVideo.do";		
      
	function goAdd()  {
		editMode="ADD";
	    openWindow("添加拍卖相关视频",fulllink+'?action=ADD&record.auctionId='+$$('auctionId').value,0.6,0.6);
	}
	
	function view(id) {
	       var url_link=fulllink+'?action=VIEW&ids='+id;
	       editMode="VIEW";
	       openWindow("查看拍卖相关视频信息",url_link,0.6,0.6);	
	}
	
	function goModify(){
		var id = findSelected("修改",ArtAuctionVideo_grid);
		if (id == "") return;
		url_link = fulllink+'?action=EDIT&record.id='+id;
		editMode="EDIT";
	    openWindow("修改拍卖相关视频信息",url_link,0.6,0.6);	  
	}

	function goDel()  {
		var id = findMultiSelected("删除",ArtAuctionVideo_grid);
		if (id == "") return;
		var res = confirm("是否要删除？");
		if (res == true){
			var data = ajaxSubmit(fulllink+"?action=delete&ids=" + id,"ArtAuctionVideoForm");
			if(data.exit>0){
	 			  alert(data.message);
	 			  return;
	 		}else {	  
	...
	function goImport() {
		url_link = fulllink+'?action=IMPORT&record.auctionId=' + $$('auctionId').value;
		openWindow("导入拍卖相关视频",url_link,0.5,0.5);
	}
...
<body onload="init()">
	<div class="special-padding" id="mainDiv">
		<s:form action="ArtAuctionVideo" id="ArtAuctionVideoForm" method="post" namespace="/auction" enctype="multipart/form-data">
			<s:hidden name="query.parameters.auctionId" id="auctionId"/>
			...
			<gl:grid styleClass="pageTurn" id="ArtAuctionVideo" page="true" form="ArtAuctionVideoForm"  property="query" cellPadding="0" cellSpacing="0">
				var grid = new dhtmlXGridObject();
         	    grid.setImagePath(imagePath);
				var headAlign = [tCenter,tCenter,tCenter,tCenter,tCenter];
				grid.setHeader("#master_checkbox,视频主题,时间,出处,链接地址",null, headAlign);
				grid.setInitWidthsP("5,40,15,20,20");
...
<script type="text/javascript">
    if(window.addEventListener) {
	 ...
	 var gDiv=window.document.getElementById('ArtAuctionVideo_box');
