...
<script language="javascript">  
	var hyperlink = "../artist/ArtistDonation.do";	
	var fulllink = contextpath + "artist/ArtistDonation.do";		
      
	function goAdd()  {
		editMode="ADD";
		var artistId = $$('artistId').value;
	    openWindow("添加作品捐赠",fulllink+'?action=ADD&record.artistId=' + artistId,0.6,0.4);
	}
	
	function view(id) {
	       var url_link=fulllink+'?action=VIEW&ids=' + id;
	       editMode="VIEW";
	       openWindow("查看作品捐赠",url_link,0.6,0.4);	
	}
	
	function goModify(){
		var id = findSelected("修改",ArtistDonation_grid);
		if (id == "") return;
		url_link = fulllink+'?action=EDIT&ids='+id;
		editMode="EDIT";
	    openWindow("修改作品捐赠",url_link,0.6,0.4);	  
	}

	function goDel()  {
		var id = findMultiSelected("删除",ArtistDonation_grid);
		if (id == "") return;
		var res = confirm("是否要删除？");
		if (res == true){
			var data = ajaxSubmit(fulllink+"?action=delete&ids=" + id,"ArtistDonationForm");
			if(data.exit>0){
	 			  alert(data.message);
	 			  return;
	 		}else {	  
	...
	function goImport() {
		...
		openWindow("导入作品捐赠",url_link,0.5,0.5);
	}
...
<body onload="init()">
	<div class="special-padding" id="mainDiv">
		<s:form action="ArtistDonation" id="ArtistDonationForm" method="post" namespace="/artist" enctype="multipart/form-data">
			...
			<gl:grid styleClass="pageTurn" id="ArtistDonation" page="true" form="ArtistDonationForm"  property="query" cellPadding="0" cellSpacing="0">
				var grid = new dhtmlXGridObject();
         	    grid.setImagePath(imagePath);
				var headAlign = [tCenter,tCenter,tCenter,tCenter];
				grid.setHeader("#master_checkbox,事件,时间,作品",null, headAlign);
				grid.setInitWidthsP("5,30,10,55");
				grid.setColAlign("center,center,center,left");
				grid.setColTypes("ch,link,ro,ro");
				grid.setColSorting("na,str,str,str");
				grid.enableMultiline(true);
				grid.init();
				var gDiv = $$('ArtistDonation_box');
			</gl:grid>
		</s:form>
	</div>
...
<script type="text/javascript">
    if(window.addEventListener) {
	 ...
	 var gDiv=window.document.getElementById('ArtistDonation_box');
