...
public class PrintExcel {
   ...
   private List<MergedPoint> 		 mergedPointList;
   ...
   public List<MergedPoint> getMergedPointList() {
	  return mergedPointList;
   }

   public void setMergedPointList(List<MergedPoint> mergedPointList) {
	  this.mergedPointList = mergedPointList;
   }
   ...
   public void doFillSheet(HSSFWorkbook wb, int sheetIndex) {
      ...
      if (printPointList != null) {
         for (PrintPoint printPoint : printPointList) {
        	HSSFRow row = sheet.getRow(printPoint.getY());
        	if(row == null)  row = sheet.createRow(printPoint.getY());
        	
            Cell cell = row.getCell(printPoint.getX());
            if(cell == null) cell = row.createCell(printPoint.getX());
            ...
            if(printPoint.getCopyCellX() > -1 && printPoint.getCopyCellY() > -1) {
            	HSSFRow copyRow = sheet.getRow(printPoint.getCopyCellY());
            	HSSFCell copyCell = copyRow.getCell(printPoint.getCopyCellX());
   ...
   private void doMerged(HSSFSheet sheet) {
	   if(this.mergedPointList == null) return;
	   
	   for(MergedPoint mp : this.mergedPointList) {
           sheet.addMergedRegion(new CellRangeAddress(mp.getBeginPointY(), mp.getEndPointY(), mp.getBeginPointX(), mp.getEndPointX()));
	   }
   }
